...
jsonMap.floors.forEach(function (f) {
    ...
    f.rooms.forEach(function (r) {
        ...
        newr = {id:r.id,floor:new_f,dummy_walls:r.dummy_walls}
        rooms.push(newr)


        r.qrs.forEach(function (q) {
            qrs.push({id:q.id,direction:q.direction,x:q.x,y:q.y,room:newr})
        })
...
jsonMap.graph.nodes.forEach(function (n) {
    ...
    {
        if (floors[i].id===n.dummy_floor_id)
            f = floors[i]
    }
...
typeLayers = {}
types = ["rooms","doors","elevators","staircases","qrs","nodes","edges"]
